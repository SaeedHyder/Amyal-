...
import com.app.amyal.global.BitmapUtil;
import com.app.amyal.global.PixelUtil;

import java.math.BigDecimal;

public class RangeSeekBarDays<T extends Number> extends ImageView {
    /**
     * Default color of a {@link RangeSeekBarDays}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
    ...
    public RangeSeekBarDays(Context context) {
        super(context);
        init(context, null);
    }

    public RangeSeekBarDays(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public RangeSeekBarDays(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.circle_price;
        int thumbPressed = R.drawable.circle_price;
        int thumbDisabled = R.drawable.circle_price;
        ...
        // make RangeSeekBarDays focusable. This solves focus handling issues in case EditText widgets are being used along with the RangeSeekBarDays within ScrollViews.
        setFocusable(true);
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            double min = Double.parseDouble(valueToString(getSelectedMinValue()));
            String minText = String.valueOf(Math.round(min))+" days";
            double max = Double.parseDouble(valueToString(getSelectedMaxValue()));
            String maxText = String.valueOf(Math.round(max))+" days";
    ...
    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBarDays widget using the {@link #setId(int)} method. Other members of this class than the normalized min and max values don't need to be saved.
     */
    @Override
    ...
    /**
     * Overridden to restore instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBarDays widget using the {@link #setId(int)} method.
     */
    @Override
     ...
     *
     * @param <T> The Number type the RangeSeekBarDays has been declared with.
     * @author Stephan Tittel (stephan.tittel@kom.tu-darmstadt.de)
     */
    public interface OnRangeSeekBarChangeListener<T extends Number> {

        void onRangeSeekBarValuesChanged(RangeSeekBarDays<T> bar, T minValue, T maxValue);
    }
