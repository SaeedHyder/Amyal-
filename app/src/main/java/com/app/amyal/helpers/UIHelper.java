...
public class UIHelper {
	...
	public static void applyDim(MainActivity mainActivityContext) {
		...
		dim.setBounds(0, 0, mainActivityContext.mainFrameLayout.getWidth(), mainActivityContext.mainFrameLayout.getHeight());
		dim.setAlpha((int) (255 * 0.7));
		ViewOverlay overlay = mainActivityContext.mainFrameLayout.getOverlay();
		overlay.add(dim);
	}
	...
	public static void showLongToastInCenter( Context ctx, String message ,boolean isRemove) {
		if(isRemove)
			removeToast();
		message = Strings.nullToEmpty( message );
		toast = Toast.makeText( ctx, message, Toast.LENGTH_LONG );
	...
	public static void showShortToastInCenter( Context ctx, String message,boolean isRemove) {
		if(isRemove)
			removeToast();
